 ...
 * Description:       A block with ACF fields that lists tools used in a makerspace.
 ...
 * Text Domain:       acf-tooling-block
 *
 * @package           acf-tooling-block
 */
 ...
 * @return void
 */
function acf_tooling_block_register() {
	register_block_type( dirname(__FILE__) . '/build/block.json' );
}
add_action( 'init', 'acf_tooling_block_register', 5 );
 ...
 * @return void
 */
function acf_tooling_block_register_include_fields() {
	...
	$field_json['location']   = array(
		array(
			array(
				'param'    => 'block',
				'operator' => '==',
				'value'    => 'acf/tooling', // block.json name.
			),
		),
	);
...
add_action( 'acf/include_fields', 'acf_tooling_block_register_include_fields' );
 ...
 */
function acf_tooling_block_block_categories( $block_categories ) {

	$block_categories = array_merge(
		[
			[
				'slug'  => 'acf-blocks',
				'title' => __( 'ACF Blocks', 'acf_tooling_block' ),
...
add_filter( 'block_categories_all', 'acf_tooling_block_block_categories' );
